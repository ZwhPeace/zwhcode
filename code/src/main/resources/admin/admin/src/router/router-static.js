import Vue from 'vue';
//配置路由
import VueRouter from 'vue-router'
Vue.use(VueRouter);
//1.创建组件
import Index from '@/views/index'
import Home from '@/views/home'
import Login from '@/views/login'
import NotFound from '@/views/404'
import UpdatePassword from '@/views/update-password'
import pay from '@/views/pay'
import register from '@/views/register'
import center from '@/views/center'
    import qiye from '@/views/modules/qiye/list'
    import discusszhaopinxinxi from '@/views/modules/discusszhaopinxinxi/list'
    import zhaopinxinxi from '@/views/modules/zhaopinxinxi/list'
    import xueshengbaoming from '@/views/modules/xueshengbaoming/list'
    import xuesheng from '@/views/modules/xuesheng/list'
    import qiyebaoming from '@/views/modules/qiyebaoming/list'
    import touzhixinxi from '@/views/modules/touzhixinxi/list'
    import xuanjianghui from '@/views/modules/xuanjianghui/list'
    import config from '@/views/modules/config/list'
    import gangwei from '@/views/modules/gangwei/list'


//2.配置路由   注意：名字
const routes = [{
    path: '/index',
    name: '首页',
    component: Index,
    children: [{
      // 这里不设置值，是把main作为默认页面
      path: '/',
      name: '首页',
      component: Home,
      meta: {icon:'', title:'center'}
    }, {
      path: '/updatePassword',
      name: '修改密码',
      component: UpdatePassword,
      meta: {icon:'', title:'updatePassword'}
    }, {
      path: '/pay',
      name: '支付',
      component: pay,
      meta: {icon:'', title:'pay'}
    }, {
      path: '/center',
      name: '个人信息',
      component: center,
      meta: {icon:'', title:'center'}
    }
      ,{
	path: '/qiye',
        name: '企业',
        component: qiye
      }
      ,{
	path: '/discusszhaopinxinxi',
        name: '招聘信息评论',
        component: discusszhaopinxinxi
      }
      ,{
	path: '/zhaopinxinxi',
        name: '招聘信息',
        component: zhaopinxinxi
      }
      ,{
	path: '/xueshengbaoming',
        name: '学生报名',
        component: xueshengbaoming
      }
      ,{
	path: '/xuesheng',
        name: '学生',
        component: xuesheng
      }
      ,{
	path: '/qiyebaoming',
        name: '企业报名',
        component: qiyebaoming
      }
      ,{
	path: '/touzhixinxi',
        name: '投掷信息',
        component: touzhixinxi
      }
      ,{
	path: '/xuanjianghui',
        name: '宣讲会',
        component: xuanjianghui
      }
      ,{
	path: '/config',
        name: '轮播图管理',
        component: config
      }
      ,{
	path: '/gangwei',
        name: '岗位',
        component: gangwei
      }
    ]
  },
  {
    path: '/login',
    name: 'login',
    component: Login,
    meta: {icon:'', title:'login'}
  },
  {
    path: '/register',
    name: 'register',
    component: register,
    meta: {icon:'', title:'register'}
  },
  {
    path: '/',
    name: '首页',
    redirect: '/index'
  }, /*默认跳转路由*/
  {
    path: '*',
    component: NotFound
  }
]
//3.实例化VueRouter  注意：名字
const router = new VueRouter({
  mode: 'hash',
  /*hash模式改为history*/
  routes // （缩写）相当于 routes: routes
})

export default router;
